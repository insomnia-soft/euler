...
int inArray(int a[], int count, int elem) {
    int i;
    for (i = 0; i < count; i++) {
        if (a[i] == elem) {
            return 1;
...
int trace() {
    ...
    while (index <= foundCount - 1) {
        col = found[index];
        for (row = col + 1; row < MAX; row++) {
            tmp = arr[row][col];
            if (tmp > 0) {
                sum += arr[row][col];
                if (!inArray(found, foundCount, row)) {
                    found[foundCount] = row;
                    foundCount++;
                }
            }
        }
        if (col > 1) {
            for (col2 = col - 1; col2 >= 0; col2--) {
                tmp = arr[col][col2];
                if (tmp > 0) {
                    if (!inArray(found, foundCount, col2)) {
                        found[foundCount] = col2;
                        foundCount++;
                    }
                }
            }
...
int isIgnored(int row, int col) {
    int i;
    for (i = 0; i < ignoreCount; i++) {
        if (ignore[i][0] == row && ignore[i][1] == col) {
            return 1;
        }
    }
    return 0;
...
void addLastRemoved() {
    arr[removedRow][removedCol] = removedVal;
    ignore[ignoreCount][0] = removedRow;
    ignore[ignoreCount][1] = removedCol;
    ignore[ignoreCount][2] = removedVal;
    ignoreCount++;
}
